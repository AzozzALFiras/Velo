...
private struct SessionTab: View {
    ...
    private var sessionDisplayName: String {
        if session.isSSHActive {
            // Extract server name from SSH connection string
            if let sshString = session.activeSSHConnectionString {
                return sshString.components(separatedBy: "@").last ?? "SSH"
            }
            return "SSH"
        }
        // Show last directory component for local sessions
        return session.currentDirectory.components(separatedBy: "/").last ?? "Terminal"
    }
