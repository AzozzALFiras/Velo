...
//  Velo
//
//  Workspace Redesign - Command Block Header
    ...
    private var contextMenuContent: some View {
        ...
        if block.isError {
            Button { onAction?(.fix) } label: {
                Label("errors.fix".localized, systemImage: "wrench.and.screwdriver")
            }
            Button { onAction?(.explain) } label: {
                Label("errors.explain".localized, systemImage: "questionmark.circle")
            }
            Divider()
        }
        ...
        Button { onAction?(.copy) } label: {
            Label("history.copyCommand".localized, systemImage: "doc.on.doc")
        }
        
        Button { onAction?(.copyOutput) } label: {
            Label("common.copy".localized, systemImage: "doc.on.clipboard")
        }
