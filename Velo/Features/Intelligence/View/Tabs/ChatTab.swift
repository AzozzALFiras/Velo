...
struct ChatTab: View {
    ...
    private var chatEmptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("intelligence.chat.empty.title".localized)
                .font(.system(size: 13, weight: .medium))
                .foregroundStyle(ColorTokens.textSecondary)

            Text("intelligence.chat.empty.subtitle".localized)
                .font(.system(size: 11))
    ...
    private var chatInputArea: some View {
        HStack(spacing: 8) {
            TextField("intelligence.chat.input.placeholder".localized, text: $chatInput)
