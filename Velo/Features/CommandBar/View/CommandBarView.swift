...
//  Velo
//
//  Workspace Redesign - Global Command Bar (⌘K)
...
struct CommandBarView: View {
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("search.noResults".localized.replacingOccurrences(of: "{}", with: searchText))
                .font(.system(size: 14))
                .foregroundStyle(ColorTokens.textSecondary)
            
            VStack(alignment: .leading, spacing: 6) {
                categoryHint(prefix: ">", label: SearchCategory.commands.label)
                categoryHint(prefix: "@", label: SearchCategory.servers.label)
                categoryHint(prefix: "/", label: SearchCategory.files.label)
                categoryHint(prefix: "?", label: SearchCategory.ai.label)
            }
            .padding(.top, 8)
        }
