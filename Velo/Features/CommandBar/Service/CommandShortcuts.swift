...
//  Velo
//
//  Workspace Redesign - Command Shortcuts/Aliases
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 8) {
                ...
                TextField("shortcuts.search".localized, text: $searchText)
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 6) {
                    CategoryPill(name: "shortcuts.all".localized, isSelected: selectedCategory == nil) {
...
struct AddShortcutSheet: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            // Header
            HStack {
                Text("shortcuts.add".localized)
                    .font(.system(size: 14, weight: .bold))
                Spacer()
                Button("theme.cancel".localized) { dismiss() }
                    .buttonStyle(.plain)
                    .foregroundStyle(ColorTokens.textTertiary)
            }
            .padding(20)
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        FieldGroup(title: "shortcuts.alias".localized, icon: "bolt.fill") {
                            TextField("e.g. pas", text: $shortcut)
                                .textFieldStyle(.plain)
                                .font(.system(size: 13, design: .monospaced))
                        }
                        
                        FieldGroup(title: "shortcuts.command".localized, icon: "terminal.fill") {
                            TextField("e.g. php artisan serve", text: $command)
                                .textFieldStyle(.plain)
                                .font(.system(size: 13, design: .monospaced))
                        }
                        
                        FieldGroup(title: "intelligence.chat.history.search.placeholder".localized, icon: "text.alignleft") {
                            TextField("intelligence.scripts.empty.subtitle".localized, text: $description)
                                .textFieldStyle(.plain)
                        }
                    }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("files.menu.getInfo.category".localized)
                    ...
                    HStack(spacing: 30) {
                        VStack(alignment: .leading, spacing: 8) {
                            Text("intelligence.scripts.icon".localized)
                                .font(.system(size: 11, weight: .semibold))
                                .foregroundStyle(ColorTokens.textTertiary)
